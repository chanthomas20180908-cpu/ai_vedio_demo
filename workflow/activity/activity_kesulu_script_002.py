...
4. Step 4：用 srt 生成 storyboard（你问的这个）
5. Step 5：根据 storyboard 的 scenes 生成每幕的图片 prompt
6. Step 6：按 scene 生图
7. Step 7：按 storyboard 合成视频

/Users/test/code/Python/AI_vedio_demo/pythonProject/.venv/bin/python3 /Users/test/code/Python/AI_vedio_demo/pythonProject/workflow/activity/activity_kesulu_script_002.py --input "/Users/test/Library/Mobile Documents/com~apple~CloudDocs/BEAST_BEING/my_mutimedia/my_scripts/唐僧克苏鲁/墙中之鼠/原文/03_split_md_20260212_104314/001_整篇__seg007.md" --image_cache_namespace "keselu_rat_20260217_001"

kesulu 002 版本目标：
1) 以字幕时间轴（SRT）为准，聚合长镜头 scene（默认 10-22s 一张图，目标 15s）
2) 每个 scene 一张图 + 简单移动动画（zoompan）合成 mp4
3) 视频不包含字幕；字幕仅用于贴图参考
4) 关键：scene 切分一定发生在字幕条目之间（不在单条字幕内部换图）
...
from workflow.taskgroup.taskgroup_image_prompts_sync_001 import image_prompts_sync_001
...
def _build_scene_prompt_request(scenes: list[dict[str, Any]]) -> str:
    # 给模型一个“每个 scene 一条 prompt”的输入
    # 保持简单：用 start/end + text
    parts: list[str] = []
    for sc in scenes:
        sid = int(sc["scene_id"])
        parts.append(
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="activity: kesulu 002（SRT分镜->生图->合成视频）")
    ...
    ap.add_argument(
        "--only_video",
        default="",
        help="仅合成视频：传入历史 out_root 目录（包含 02_image_prompts/storyboard 与 04_audio/*.wav 与 03_images/scene_*.png）。",
    )
    ...
    ap.add_argument(
        "--cloned_voice_id",
        default="cosyvoice-v3-flash-manflash01-7cc91b1194ed4a4a982d035734709b8b",
    )
    ...
    if args.only_video:
        ...
        compose_video_from_storyboard_001(
    ...
    try:
        ...
        # 经验规则：cosyvoice-v3-flash 在传 --instruction 时，服务端对 SSML(<speak>/<break>) 兼容性较差，
        # 因此这里自动切到纯文本（等价于手动加 --no_break），避免 418/Invalid payload data。
        ...
        # =========================
        # Step 5: prompts aligned with scenes (single call)
        # =========================
        ...
        if bool(args.skip_images):
            ...
            # 更新 storyboard（补全 prompt/image_path）
            write_storyboard_json(storyboard_path, payload)
...
/Users/test/code/Python/AI_vedio_demo/pythonProject/.venv/bin/python3 \
  /Users/test/code/Python/AI_vedio_demo/pythonProject/workflow/activity/activity_kesulu_script_002.py \
  --input "/path/to/xxx.md" \
  --output_root "/Users/test/code/Python/AI_vedio_demo/pythonProject/data/Data_results/script_results" \
  --target_sec 15 --min_sec 10 --max_sec 22 --gap_sec 0.8 \
